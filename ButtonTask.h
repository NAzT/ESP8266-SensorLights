...
{
public:
    typedef void(*action)(ButtonState state, uint8_t sensorno);

    ButtonTask(action function, uint8_t sensorno) :
        Task(MsToTaskTime(3)), // check every three millisecond, 1-10 ms should be ok
        _sensorno(sensorno),
        _buttonPin(sensorpin[sensorno]),
        _callback(function)
    { 
    };
    ...
    virtual void OnUpdate(uint32_t deltaTime)
    {
        ...
        if (pinState != (_state & ButtonState_Pressed))
        {
            ...
            else
            {
                ...
                {
                    // triggered released
                    _callback(ButtonState_Released,_sensorno);
                }
                _state = ButtonState_Released;
            }
        }
        else
        {
            ...
            case ButtonState_Tracking:
                if (deltaTimeMs >= _timer)
                {
                    ...
                    _callback(ButtonState_Pressed, _sensorno);
                }
                else
                {
                    _timer -= deltaTimeMs;
          ...
          // auto repeat started
                    ...
                    _callback(ButtonState_AutoRepeat, _sensorno);
                }
                else
                {
                    _timer -= deltaTimeMs;
